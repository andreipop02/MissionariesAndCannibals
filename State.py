...
class State(object):
	...
	def successors(self):
		...
		if self.dir == Direction.OLD_TO_NEW:
			sgn = -1
			direction = "din partea stanga in partea dreapta a raului"
		else:
			sgn = 1
			direction = "din partea dreapta in partea stanga a raului"
		for i in self.moves:
			(m, c) = i
	...
	def addValidSuccessors(self, listChild, m, c, sgn, direction):
		...
		if newState.isValid():
			newState.action = " luam %d misionari si %d canibali %s." % (m, c, direction)
